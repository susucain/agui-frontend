...
SSH_KEY="${SSH_KEY:-$HOME/store/aliyun/my-key.pem}"
...
echo ""
echo "[3/5] 上传镜像到 ECS..."
ssh -i ${SSH_KEY} -p ${ECS_PORT} ${ECS_USER}@${ECS_HOST} "mkdir -p /tmp/deploy ${REMOTE_DIR}"
scp -i ${SSH_KEY} -P ${ECS_PORT} ${TAR_FILE} ${ECS_USER}@${ECS_HOST}:/tmp/deploy/
echo "  ✓ 上传完成"

# ---- Step 4: ECS 上同步源码 + 部署 ----
echo ""
echo "[4/5] 同步源码并部署..."
ssh -i ${SSH_KEY} -p ${ECS_PORT} ${ECS_USER}@${ECS_HOST} << ENDSSH
set -e

# -- 4a. 检查 git 是否已安装 --
if ! command -v git &>/dev/null; then
    echo "  ❌ ECS 上未安装 git，请先执行: yum install -y git"
    exit 1
fi
